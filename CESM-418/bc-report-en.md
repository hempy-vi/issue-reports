...
**1. Dev lead instructed switching to "monthly-closing mode" for July+August — checked before the user acted**

Six days later (the "no data showing" issue was still unresolved — see item 12 below), the matter escalated to executive level, with the customer requesting a same-day confirmation call from the Korean boss. The dev lead instructed: rerun the "monthly-closing mode" (instead of the daily mode currently in use) for both July and August, and clear the delivery-tracking data so it re-maps — reasoning: since the customer had already closed their books for August, the closing-mode mechanism should be used.

Before the user acted, checked the real completeness of the snapshot data source that "monthly-closing mode" relies on, against the true figures in the lot-tracking system: confirmed **July's snapshot source is now 100% complete** (it had caught up since the earlier check), but **August's is still only about 52% complete, missing nearly half**. Clearly reported to the user: running this mode for July would be safe, but for August it would cause real data loss — exactly the risk already flagged earlier; the reasoning "already closed the books, so closing mode must be used" does not correctly apply to this system, since the snapshot source is generated by a completely separate process and does not reflect the lot-tracking module's own data completeness.

**2. User decided to proceed with the dev lead's instruction anyway**

Under urgent business pressure (the customer required same-day confirmation), the user proactively followed the instruction: cleared July-August-September data (both the material-tracking and delivery-tracking portions), then reran using "monthly-closing mode" for July and August, and the normal mode for September. Note: no rebuild step for the delivery-tracking data was included after the clear.

**3. Verified the actual damage after execution — confirmed as predicted, plus 2 unanticipated cascading effects**

| Month | Before | After | Assessment |
|---|---|---|---|
| 07 (closing mode) | — | Matched the true figures exactly | Safe, nothing lost |
| 08 (closing mode) | ~1.8 million kg of carried-forward inventory | Only ~119 thousand kg left | **Real loss of about 93% of the carried-forward inventory quantity** |
| 09 (normal mode) | ~2.9 million kg of carried-forward inventory | **Completely zero** | Cascading effect — September's figures are computed from August's data, which had just been lost |
| Delivery-tracking data (Jul-Sep) | 13/141 lots with data | **Completely empty (0/141)** | Because the clear step had no rebuild step included with it |
