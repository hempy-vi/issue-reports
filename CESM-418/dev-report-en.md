...
**1. Dev lead instructed running the MM branch for July+August — verified before the user executed it**

Six days later (the "No data" issue was still unresolved — see item 12 below), the matter escalated to executive level. The dev lead (Doan Giau) instructed via internal chat: rerun `LG_PRO_DAILY_LOT_TRACKING` with `P_TYPE='MM'` for both July and August, plus delete `TLG_LOT_TRACKING_GD_D`/`GD_M` so it re-maps — reasoning given: "August has already been closed by them" (so the monthly-closing mechanism should be used).

Before the user executed this, independently re-verified the real coverage of `TLG_CL_CLOSING_MAT_DETAIL` (the data source the MM branch uses) against the true mixing-lot count in `TLG_LOT_TRACKING_MAT`:
```sql
...
GROUP BY M.STD_YM;
```
Result: **July — snapshot 92 lots = true count 92 lots (100%, had caught up since the 09-08 check)**; **August — snapshot still only 90 lots, true count 173 lots (~52%, still significantly short)**. Clearly reported to the user: running MM for July would be safe, but for August it would cause real data loss (~48% of mixing lots) — exactly the risk already warned about on 09-08; the dev lead's "already closed" reasoning does not apply to this system, since the closing snapshot is generated by a completely different pipeline and does not reflect Lot Tracking's own data completeness.

**2. User decided to proceed with the dev lead's instruction anyway**

Under business pressure (the customer had requested the Korean boss call to confirm that same day), the user proactively ran:
```sql
...
COMMIT;
```
Note: the script included no rebuild call for GD_M/GD_D after the delete.

**3. Verified the actual damage after execution — confirmed exactly as predicted, plus 2 unanticipated cascading effects**
...
| Month | Before | After | Assessment |
|---|---|---|---|
| 07 (MM) | — | OPEN 90 rows/114,667.79kg (matches the now-complete snapshot exactly; DAILY/MAPPING also match the true totals 1,094,801.34/1,090,192 exactly) | Safe, nothing lost |
| 08 (MM) | OPEN 899 rows/1,805,854.78kg | OPEN 62 rows/119,277.13kg | **Real loss of ~1,686,577.65kg (93.4%)** |
| 09 (DD) | OPEN 1,326 rows/2,890,497.47kg | **OPEN = 0 rows, completely empty** | Cascading effect (September's carry-over is computed from August's data, which had just been wiped) |
| GD_M/GD_D (Jul-Sep) | 13/141 lots with data | **0/0 — completely empty** | The script only deleted, with no rebuild call afterward |

**4. Identified the recovery path — no need to start over**

Since July (even though run via MM) happened to produce the true correct figures (July's snapshot is now fully complete), the underlying source data August's carry-over would need to recompute via DD remains intact and correct. Simply rerun **DD** (not MM) for August+September — no manual DELETE needed first, since the DD branch automatically soft-deletes that month's data as its first step — then rebuild GD_M/GD_D:
```sql
